...
class OSINTToolkit:
    ...
    def _print_menu_with_proxy(self):
        ...
        console.print(Panel("\n".join(menu_items), title="[bold cyan]🎯 OSINT Toolkit Pro v3.1[/bold cyan]", border_style="cyan", box=box.ROUNDED))
    ...
    def _username_mode(self):
        ...
        console.print("[cyan]Выберите инструмент:[/cyan]")
        if self.username_checker._maigret_available:
            console.print("[green]1.[/green] Maigret (3000+ сайтов)")
        if self.username_checker._sherlock_available:
            console.print("[green]2.[/green] Sherlock (быстрый)")
        
        default = "1" if self.username_checker._maigret_available else "2"
        tool = Prompt.ask("[bold yellow]Выбери инструмент[/bold yellow]", default=default)
        ...
        if use_sherlock:
            ...
            console.print("[cyan]Режимы поиска:[/cyan]")
            console.print("[green]1.[/green] Быстрый (топ-509 сайтов)")
            console.print("[green]2.[/green] Полный (все 3000+ сайтов)")
            mode = Prompt.ask("[bold yellow]Выбери режим[/bold yellow]", default="1")
            full_search = mode == "2"
            results = self.username_checker.run(username, use_sherlock=False, full_search=full_search)
    ...
    def _all_in_one_mode(self):
        ...
        if email and "@" in email and self.email_checker.is_available():
            print_info(f"Проверяем email: {email}")
        ...
        if username and self.username_checker.is_available():
        ...
        if phone and self.phone_checker.is_available():
        ...
        if domain and self.domain_checker.is_available():
            print_info(f"Анализируем домен: {domain}")
            result = self.domain_checker.run(domain)
            if result.get("ip"):
    ...
    def _batch_mode(self):
        ...
        for line in lines:
            if ':' in line:
                ...
                value = value.strip()
                if mode == "email" and "@" in value and self.email_checker.is_available():
                ...
                elif mode == "username" and self.username_checker.is_available():
                ...
                elif mode == "phone" and self.phone_checker.is_available():
                    print_info(f"Phone: {value}")
                    result = self.phone_checker.run(value)
                elif mode == "domain" and self.domain_checker.is_available():
